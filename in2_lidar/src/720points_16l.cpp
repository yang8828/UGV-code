...
#define CarLeftEdge	      1.045 //0.945
...
void processScan( const VPointCloud::ConstPtr &scan_msg )
{
	...
	for(int i = 0; i < 720; i++)
	{
		...
		for(j = 1; j < boxes[i].PointsCount; j++)
		{
			...
			//if( angle >  0.4 && boxes[i].z[j]-boxes[i].z[j-1] > 0.10 )
			{
			...
			//if( angle <  -0.2 && boxes[i].z[j]-boxes[i].z[j-1] < -0.10 )
			{
				finalbox.x[finalbox.PointsCount] = boxes[i].x[j-1];
				finalbox.y[finalbox.PointsCount] = boxes[i].y[j-1];			
				finalbox.norm[finalbox.PointsCount] = norm;
	...
	for(int i = 0; i < 720; i++)
	{
		scaninfov2_msg.ScanInfoX[i] = (long)(1500 - finalbox.y[i]*50.0);
		scaninfov2_msg.ScanInfoY[i] = (long)(1000 + finalbox.x[i]*50.0);

		//0.02m	
		//scaninfov2_msg.ScanInfoX[i] = (long)( -finalbox.y[i]*50 );
		//scaninfov2_msg.ScanInfoY[i] = (long)(  finalbox.x[i]*50 );
	}
	...
	{	
		...
		line( dismap,
			  ...
			  Point( offset_map_x + scaninfov2_msg.ScanInfoX[(i+1)]/(50*display_range/grid_dim_), -offset_map_y + grid_dim_ - scaninfov2_msg.ScanInfoY[(i+1)]/(50*display_range/grid_dim_) ),
			  Scalar(255,0,0),
			  1
			);
...
{
	ros::init(argc, argv, "velodyne16l_node");

	ros::NodeHandle node;
	my_servertime.C_ServerTime_Init();

	node.param("/velodyne16l_node/cell_size", m_per_cell_, 0.05);
	node.param("/velodyne16l_node/grid_dimensions", grid_dim_, 600);
	node.param("/velodyne16l_node/display_range", display_range, 90.0);

	node.param("/velodyne16l_node/height_nega_coe", height_nega_coe, 27.0);
	node.param("/velodyne16l_node/height_nega_min", height_nega_min, 0.07);
	node.param("/velodyne16l_node/height_nega_max", height_nega_max, 0.12);

	node.param("/velodyne16l_node/height_posi_coe", height_posi_coe, 27.0);
	node.param("/velodyne16l_node/height_posi_min", height_posi_min, 0.05);
	node.param("/velodyne16l_node/height_posi_max", height_posi_max, 0.10);

	node.param("/velodyne16l_node/angle_nega_coe", angle_nega_coe, 27.0);
	node.param("/velodyne16l_node/angle_nega_min", angle_nega_min, 0.20);
	node.param("/velodyne16l_node/angle_nega_max", angle_nega_max, 0.25);

	node.param("/velodyne16l_node/angle_posi_coe", angle_posi_coe, 27.0);
	node.param("/velodyne16l_node/angle_posi_min", angle_posi_min, 0.40);
	node.param("/velodyne16l_node/angle_posi_max", angle_posi_max, 0.45);

	node.param("/velodyne16l_node/deadzone_nega", deadzone_nega, 10.0);
	node.param("/velodyne16l_node/deadzone_posi", deadzone_posi, 10.0);

	node.param("/velodyne16l_node/showimages",   showimages,   false);
	node.param("/velodyne16l_node/showmessages", showmessages, false);

	ros::Subscriber scan;
	scan = node.subscribe("rotate_16l", 10, &processScan,  ros::TransportHints().tcpNoDelay(true) );

	scanInfov2_pub = node.advertise<in2_msgs::ScanInfoV2>("ScanInfoV2_16l", 1);
