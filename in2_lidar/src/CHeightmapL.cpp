...
CHeightmap::CHeightmap(ros::NodeHandle node, ros::NodeHandle priv_nh)
{
	...
	node.param("/heightmap16l_node/showmessages", showmessages, false);
    node.param("/heightmap16l_node/showimages",   showimages,   false);

	// Set up publishers  
	image_transport::ImageTransport it(node);
	hm_publisher = it.advertise("heightmap16l",1);
	
	// subscribe to Velodyne data points
	//velodyne_scan_ = node.subscribe("velodyne_points", 10,
	velodyne_scan_ = node.subscribe("rotate_16l", 10,
...
void CHeightmap::processData(const VPointCloud::ConstPtr &scan)
{
	...
	if(showimages)
	{
		imshow("heightmap16l",HeightMapMat);
		waitKey(1);
	}

	//publish heightmap topic 
	std_msgs::Header header0;
	header0.frame_id = "heightmap16l";
