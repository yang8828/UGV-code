...
void integrateCloud()
{
			...
			cloud_pc_whole.header.frame_id = "base_ibeo_4L";
...
{
	ros::init(argc, argv, "ibeo_4L_node");
	ros::NodeHandle node;
	 ros::NodeHandle n_tf;
  ...
  tf::TransformBroadcaster broadcaster;
	...
	scan_pub_ = node.advertise<sensor_msgs::PointCloud>("ibeo_4L_scan", 1000);
	objects_pub_ = node.advertise<in2_msgs::IbeoObjects>("ibeo_4L_objects", 1000);

	CIbeoLuxETH ibeo_lux_4L("192.168.0.20",12002);
	IBEO_TYPE = IBEO_4L_LUX;
	ibeo_lux_4L.initialize();

	while(ros::ok())
	{
			
		ibeo_lux_4L.doProcess();
		if(data_flag)
		{
			integrateCloud();
        ...
        tf::StampedTransform(
        tf::Transform(tf::Quaternion(0, 0, 1, 0), tf::Vector3(0, 0, 0)),
        ros::Time::now(),"base_vehicle", "base_ibeo_4L"));
		}
