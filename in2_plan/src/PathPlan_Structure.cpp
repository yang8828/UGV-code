...
bool in_shi_gong_last = false;
long int in_shi_gong_time = 0;
bool rm_shi_gong = false;
bool is_start_car = false;
...
// ****** line to line filter ****** //
	if (1||is_sent_pts_last_init) {
		...
		double lazy_coe = (100.0-lanechangespeed)*0.01;
...
void imageObsConvert(const sensor_msgs::ImageConstPtr& msg)
{
	...
	*/
	...
	if (IsEstop3 && !IsRedLight() && !in_shi_gong) estop_3_cnt++;
	else                                           			    estop_3_cnt = 0;
...
void transferGISRect(const in2_msgs::GisInfo::ConstPtr &gisRect_msg)
{
	...
	if (gisRect.LaneChangeFlag == 3 && !rm_shi_gong)  in_shi_gong = true;
	else                                              in_shi_gong = false;
	
	
	// pedestrian
	if (gisRect.LaneChangeFlag == 2) is_in_pedestrian = true;
	else                              is_in_pedestrian = false;
	
	// suidao
	if (gisRect.LaneChangeFlag == 4) is_in_suidao = true;
...
long Caculatespeed2()
{
	...
	if (1) {
		...
		if (1) {
			...
			double estop_dis = 10.0;
			
			if (is_in_pedestrian || is_in_suidao) {
				estop_dis = 13.0;
			}
			
			if (in_shi_gong) {
				estop_dis = 30.0;
				if (fabs(Cur_EV_Speed) < 2.0) {
					in_shi_gong_time++;
					if (in_shi_gong_time > 350) {
						rm_shi_gong = true;
					}
				} else {
					in_shi_gong_time = 0;
			...
			if (g_distancefront > 4995.0) {
				speedsenttemp = max_speed_temp * 1000.0;
			} else if (g_distancefront <= 4995.0 && g_distancefront > 4500) {
...
void display()
{
	for(int i=0;i<300;i++)
	{
		cvLine(PlanMapShow,cvPoint(Gis_Lane[i].x,(3000-Gis_Lane[i].y)),cvPoint(Gis_Lane[i+1].x,(3000-Gis_Lane[i+1].y)),CV_RGB(255,0,255),10); // ZI SE
	...
	sprintf(strtemp,"is_in_suidao: %d", is_in_suidao);
		cvPutText(PlanMapShow,strtemp,cvPoint(50,1900),&font,CV_RGB(0,255,0));
	sprintf(strtemp,"is_in_pedestrian: %d", is_in_pedestrian);
		cvPutText(PlanMapShow,strtemp,cvPoint(50,2000),&font,CV_RGB(0,255,0));
	sprintf(strtemp,"in_shi_gong: %d", in_shi_gong);
	...
	case 2:
		for(int i=0;i<planlanenumber;i++)
		{
			if(i==SelectPlanPathIndex)
			{
				for(int j=0;j<300;j++)
				{
					cvLine(PlanMapShow,cvPoint(plines[i][j].x,(3000-plines[i][j].y)),cvPoint(plines[i][j+1].x,(3000-plines[i][j+1].y)),CV_RGB(0,255,255),10); // QING SE
				}
			}
			else
			{
	...
	for(int j=0;j<300;j++)
	{
		cvLine(PlanMapShow,cvPoint(roadpts_Sent[j].x,(3000-roadpts_Sent[j].y)),cvPoint(roadpts_Sent[j+1].x,(3000-roadpts_Sent[j+1].y)),CV_RGB(255,0,0),20); // HONG SE
